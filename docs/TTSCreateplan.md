...
## 13. 2026-06-29 macOS M1 테스트 대응 수정 기록

### 문제 현상

- 이전 수정은 Windows Editor에서 PowerShell과 `System.Speech` 기반으로만 실제 TTS를 출력했다.
- MacBook M1의 Unity Editor에서는 Windows PowerShell 백엔드가 실행될 수 없으므로, 음악 ducking만 보이고 대사 출력은 실패한다.

### 코드 변경

- `TtsBackend`에 `MacOsSay`와 `Auto`를 추가했다.
- `TtsController`의 기본 백엔드를 `Auto`로 변경했다.
- `Auto` 선택 시 플랫폼별 기본 백엔드를 자동 생성한다.
  - macOS Editor/Standalone: `MacOsSayTtsService`
  - Windows Editor/Standalone: `WindowsPowerShellTtsService`
  - 기타 플랫폼: `LogTtsService`
- `MacOsSayTtsService`를 추가해 macOS 기본 `/usr/bin/say` 명령으로 TTS를 재생한다.
- 한국어 문장에 한글이 포함되어 있으면 `say -v ?` 목록에서 `ko_KR`/`ko-KR` 음성을 찾아 우선 사용한다.
- `Stop` 버튼은 실행 중인 `say` 프로세스를 종료하도록 연결했다.
- `Assets/Scenes/TtsDemo.unity`의 `TtsController.backend` 값을 `Auto`로 변경했다.

### MacBook M1 수동 확인 항목

MacBook M1에서 Unity Editor로 `Assets/Scenes/TtsDemo.unity`를 열고 아래를 확인한다.

1. Inspector에서 `TTS Runtime > TtsController > Backend`가 `Auto`인지 확인한다.
2. Play Mode 진입 후 `Test Music On`을 눌러 음악을 재생한다.
3. 한국어 코칭 문장을 입력하고 `Speak`를 누른다.
4. macOS 시스템 음성으로 대사가 들리고, 동시에 테스트 음악이 낮아지는지 확인한다.
5. `Stop`을 누르면 대사가 멈추고 음악 볼륨이 복구되는지 확인한다.
6. 대사가 들리지 않으면 터미널에서 `say -v ? | grep -i ko`로 한국어 음성이 설치되어 있는지 확인한다.
