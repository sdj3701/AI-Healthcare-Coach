...
3. 같은 자세 오류가 반복 구간의 최소 프레임과 비율을 넘지 않음
4. `ExercisePhaseDetector`의 rep count가 증가함
5. 현재 rep가 오류 없는 rep이면 `CorrectRepCount` 증가

한 번 나타났다가 교정된 경고는 정확 카운트를 막지 않는다. 정확 카운트가 증가하지 않은 경우에는 확정 자세 규칙을 세션별로 집계해 결과 화면에서 원인을 표시한다.

## 세트 휴식, 목표 완료, 결과

- 목표 설정 화면에서 반복, 세트와 세트 간 휴식 시간을 함께 설정한다.
- 테스트 기본 세트 휴식은 3초다.
- 목표는 전체 시도 횟수가 아니라 `CorrectRepCount`에 적용한다.
- 각 세트의 정확 반복을 채우면 마지막 세트가 아닌 경우 추적과 TTS를 멈추고 휴식 카운트다운을 시작한다.
- 휴식 종료 후 카메라 추적을 자동 재개한다. 세트 전환으로 phase 상태가 초기화되어도 `TotalRepCount`, `CorrectRepCount`, 자세 문제와 운동 시간은 유지한다.
- 마지막 세트를 채우면 추가 휴식 없이 운동을 종료하고 전체/정확 반복, 운동 시간, 예상 칼로리, 운동 점수, 주요 자세 문제를 표시한다.
- `2회 × 2세트`의 실행 순서는 `2회 → 휴식 → 2회 → 결과`다.
- 운동 점수는 `정확 반복 / 전체 반복 × 100`, 칼로리는 `운동별 kcal/회 × 전체 반복` 추정값이다.
- 결제 계층은 구매 완료 시 `MobileWorkoutPrototypeView.SetPaidStretchingAccess(true)`를 호출한다. `ExpertContent` 권한이 있으면 주요 자세 문제에 맞춘 회복 스트레칭을 보여 준다.
...
7. 결과 기록의 영구 저장 및 기간별 추세 추가
8. 실제 스토어 구매/복원 SDK와 스트레칭 권한 연결
9. 모바일 해상도별 레이아웃 QA
